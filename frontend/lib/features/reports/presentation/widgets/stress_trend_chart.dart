...
class StressTrendChart extends StatelessWidget {
  ...
  const StressTrendChart({super.key, required this.trends});
  ...
  Widget build(BuildContext context) {
    ...
    if (trends.isEmpty) {
      return Container(
        ...
        child: Text(
          'No stress records available.',
          style: theme.textTheme.bodyMedium?.copyWith(color: theme.colorScheme.onSurface.withOpacity(0.5)),
        ),
      );
    }

    final List<FlSpot> stressSpots = trends.asMap().entries.map((MapEntry<int, TrendData> entry) {
      final double idx = entry.key.toDouble();
      final double stress = entry.value.stressLevel.toDouble();
      return FlSpot(idx, stress);
    }).toList();
            ...
            LineChartBarData(
              ...
              belowBarData: BarAreaData(
                ...
                gradient: LinearGradient(
                  ...
                  colors: <Color>[
                    Colors.orange.shade600.withOpacity(0.18),
                    Colors.orange.shade600.withOpacity(0.01),
                  ],
                ),
              ),
            ),
